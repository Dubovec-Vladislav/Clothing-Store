...
export const Filters: FC<FiltersProps> = ({
  ...
  data?.forEach((item) =>
    item.sizesList.forEach(
      (size) => !sizesList.includes(size) && sizesList.push(size)
    )
  ); // Get all sizes of clothing on the page

  const handleColorClick = (i: number) => {
    const color = colorsList[i];
    selectedColorsList.includes(color)
      ? changeSelectedColorsList(
          selectedColorsList.filter((item) => item !== color)
      ...
      <div className={style.body}>
        <div className={style.title}>
          ...
          <div
            ...
            onClick={() => toggleIsFilteringMenuActive(false)}
          ></div>
        </div>
