...
import {
  ...
  getClothingItemsBySearchAndPage,
  getClothingItems,
} from "app/commonApi";
// Slice
...
export const ClothingContent: FC<ClothingContentProps> = ({ pageName }) => {
  ...
  const categoriesList = useMemo(
    () => ["Деловая", "Повседневная", "На вечеринку", "Для зала"],
    []
  );
  const [activeTitle, setActiveTitle] = useState<string>("");
  useEffect(() => {
    id
      ? setActiveTitle(categoriesList[Number(id) - 1])
      : setActiveTitle("Поиск");
  }, [id, categoriesList]);
  // --------------------------------------------- //
  ...
  const [isFilteringMenuActive, toggleIsFilteringMenuActive] =
  ...
  // Filtering by price
  const dataSortedByPrice = filterData(
    data,
  ...
  const sortedData: ClothingInterface[] = getCommonVariantsFromArrays(
    generalArray,
