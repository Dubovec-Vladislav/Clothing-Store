...
export const FilterPopup: FC<FilterPopupProps> = ({
  indexOfActiveSortType,
  setIndexOfActiveSortType,
  activeSortTypeName,
  sortTypes,
}) => {
  ...
  return (
    <div className={style.block} ref={sortRef}>
      <div
        ...
        onClick={() => toggleIsPopupActive(!isPopupActive)}
      >
        <div className={style.text}>
          Порядок сортировки: <span>{activeSortTypeName}</span>
        </div>
        <div
          className={
            isPopupActive
              ? `${style.arrow} ${style.activeArrow}`
              : `${style.arrow}`
          }
        >
