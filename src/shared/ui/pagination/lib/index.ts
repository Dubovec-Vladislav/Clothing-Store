      ...
      pageNumber => (
        ...
        || (currentPage < 4
          ...
          : currentPage > arrayOfPages.length - 4
            ? (pageNumber >= arrayOfPages.length - 5 && pageNumber <= currentPage + 2) // Or if the page is 4 pages before the last one, then increase the left border (Optional)
            : (pageNumber >= currentPage - 2 && pageNumber <= currentPage + 2)) // Or this page is within +/- 2 pages of the current page
