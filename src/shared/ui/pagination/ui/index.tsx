...
export const Pagination: FC<PaginationProps> = ({
  ...
  const arrayOfPages =
    dataLength && defineNumberOfPages(dataLength, pageLimit, currentPage);
      ...
      <div className={style.pageNumbers}>
        {isLoading ? (
          <div style={{ padding: "18px 0px 0px 10px" }}>
            Идет загрузка страниц...
          </div>
        ) : arrayOfPages ? (
          arrayOfPages.map((pageNumber, i) =>
            pageNumber === 0 ? (
              <div key={i} className={`${style.page} ${style.ellipsis}`}>
                ...
              </div>
            ) : pageNumber === currentPage ? (
              <div
                key={i}
                className={`${style.page} ${style.activePage}`}
